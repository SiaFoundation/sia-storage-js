...
- `GITHUB_TOKEN` — built-in, used by Knope to tag and create the release.

> **Gotcha:** PRs opened by `GITHUB_TOKEN` don't trigger other workflows. The Release PR created by `prepare-release` won't auto-run CI. Close and reopen it to kick off a run, or add a `PAT` secret and switch `peter-evans/create-pull-request` to use it.
