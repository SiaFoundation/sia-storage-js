...
describe('rollup bundler integration', () => {
  ...
  beforeAll(async () => {
    ...
    // Rollup leaves the new URL("…wasm", import.meta.url) reference in place
    // and doesn't emit assets without an extra plugin. Real-world setups
    // either copy the .wasm explicitly or use @rollup/plugin-url. Choosing
    // explicit copy here for parity with the esbuild test.
    cpSync(
